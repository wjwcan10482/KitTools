...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		...
		apt-get install -y xz-utils openssl gawk file wget curl || apt install -y xz-utils openssl gawk file wget curl
		sleep 3s
	fi
else
    echo '当前系统 是 CentOS/Rocky/Oracle/RHEL'
    echo 'Current system is CentOS/Rocky/Oracle/RHEL'
    yum install -y xz openssl gawk file wget curl ||  dnf install -y xz openssl gawk file wget curl
    sleep 3s
fi
...
# fi
CentOSMirrors="--mirror http://178.105.97.4:81/CentOS/"
CentOSVaultMirrors="--mirror http://178.105.97.4:81/CentOS/"
DebianMirrors="--mirror http://mirrors.aliyun.com/debian/"
UbuntuMirrors="--mirror http://mirrors.aliyun.com/ubuntu/"
...
echo "                                                                "
  ...
  case $N in
    ...
    2) echo -e "\nInstall...CentOS 7(ARM64)\n"; read -s -n1 -p "任意键继续(Press any key to continue...)" ; bash ~/Core_Install.sh -a -v arm64 -dd "http://178.105.97.4:81/CentOS_7.X_ARM64_UEFI_NetInstallation_Final_v9.11.vhd.gz" $DebianMirrors $CXTaddLine ;;
...
echo "                                                                "
  ...
  read N
  case $N in
    1) echo -e "\nInstall...CentOS 9\n"; read -s -n1 -p "任意键继续(Press any key to continue...)" ; bash ~/Core_Install.sh -a -v 64 -dd "http://178.105.97.4:81/CentOS_9.X_x64_Legacy_NetInstallation_Stable_v1.6.vhd.gz" $DebianMirrors $CXTaddLine ;;
    2) echo -e "\nInstall...CentOS 8\n"; read -s -n1 -p "任意键继续(Press any key to continue...)" ; echo -e "\nbash -x ~/Core_Install.sh -a -v 64 -dd "http://178.105.97.4:81/CentOS_8.X_x64_Legacy_NetInstallation_Stable_v6.8.vhd.gz" $CentOSVaultMirrors $CXTaddLine \n";echo "$CentOSVaultMirrors \n" ;;
    3) echo -e "\nInstall...CentOS 7\n"; read -s -n1 -p "任意键继续(Press any key to continue...)" ; bash ~/Core_Install.sh -a -v 64 -dd "http://178.105.97.4:81/CentOS_7.X_x64_Legacy_NetInstallation_Final_v9.8.vhd.gz" $CentOSVaultMirrors $CXTaddLine ;;
